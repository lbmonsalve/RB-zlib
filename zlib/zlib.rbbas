...
Protected Module zlib
	...
	#tag Method, Flags = &h1
		Protected Function Compress(Data As MemoryBlock, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default, DataSize As Integer = - 1) As MemoryBlock
		  ...
		  Do
		    If Level = CompressionLevel.Default Then
		      err = _compress(OutBuffer, OutSize, Data, DataSize)
		    Else
		      err = _compress2(OutBuffer, OutSize, Data, DataSize, Integer(Level))
		    End If
		    Select Case err
		    Case Z_STREAM_ERROR
		      Break ' Level is invalid; using default
		      Return Compress(Data, zlib.CompressionLevel.Default, DataSize)
	...
	#tag Method, Flags = &h1
		Protected Function Deflate(Source As FolderItem, Destination As FolderItem, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default, Overwrite As Boolean = False, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As Boolean
		  ...
		  Dim ok As Boolean
		  Try
		    ' calls Deflate(Readable, Writeable, CompressionLevel, CompressionType) As Boolean
		    ok = Deflate(src, dst, Level, Encoding)
		  Finally
		    src.Close
		    dst.Close
		  End Try
	...
	#tag Method, Flags = &h1
		Protected Function Deflate(Source As FolderItem, Destination As Writeable, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As Boolean
		  ...
		  Try
		    ' calls Deflate(Readable, Writeable, CompressionLevel, CompressionType) As Boolean
		    ok = Deflate(src, Destination, Level, Encoding)
		  Finally
		    src.Close
		  End Try
		  Return ok
	...
	#tag Method, Flags = &h1
		Protected Function Deflate(Source As FolderItem, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As MemoryBlock
	...
	#tag Method, Flags = &h1
		Protected Function Deflate(Source As MemoryBlock, Destination As FolderItem, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default, Overwrite As Boolean = False, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As Boolean
		  ...
		  Dim ok As Boolean
		  Try
		    ' calls Deflate(MemoryBlock, Writeable, CompressionLevel, CompressionType) As Boolean
		    ok = Deflate(Source, dst, Level, Encoding)
		  Finally
		    dst.Close
		  End Try
		  Return ok
	...
	#tag Method, Flags = &h1
		Protected Function Deflate(Source As MemoryBlock, Destination As Writeable, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As Boolean
		  ' Compress the Source data into the Destination stream. Use Inflate to reverse.
		  ' See: https://github.com/charonn0/RB-zlib/wiki/zlib.Deflate
		  
		  Dim src As New BinaryStream(Source)
		  ' calls Deflate(Readable, Writeable, CompressionLevel, CompressionType) As Boolean
		  Return Deflate(src, Destination, Level, Encoding)
		End Function
	#tag EndMethod

	#tag Method, Flags = &h1
		Protected Function Deflate(Source As MemoryBlock, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As MemoryBlock
		  ...
		  Dim src As New BinaryStream(Source)
		  ' calls Deflate(Readable, Writeable, CompressionLevel, CompressionType) As Boolean
		  If Not Deflate(src, dst, Level, Encoding) Then Return Nil
		  dst.Close
		  Return buffer
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h1
		Protected Function Deflate(Source As Readable, Destination As FolderItem, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default, Overwrite As Boolean = False, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As Boolean
	...
	#tag Method, Flags = &h1
		Protected Function Deflate(Source As Readable, Destination As Writeable, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As Boolean
		  ...
		  Dim z As ZStream = ZStream.Create(Destination, Level, Encoding)
		  Try
		    Do Until Source.EOF
	...
	#tag Method, Flags = &h1
		Protected Function Deflate(Source As Readable, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As MemoryBlock
		  ...
		  ' calls Deflate(Readable, Writeable, CompressionLevel, CompressionType) As Boolean
		  If Not Deflate(Source, stream, Level, Encoding) Then Return Nil
		  stream.Close
		  Return buffer
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h1
		Protected Function GUnZip(Source As FolderItem) As MemoryBlock
		  ...
		  ' calls Inflate(FolderItem, MemoryBlock, CompressionType) As MemoryBlock
		  Return Inflate(Source, Nil, CompressionType.GZip)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h1
		Protected Function GUnZip(Source As FolderItem, Destination As FolderItem, Overwrite As Boolean = False) As Boolean
		  ...
		  ' calls Inflate(FolderItem, FolderItem, Boolean, MemoryBlock, CompressionType) As Boolean
		  Return Inflate(Source, Destination, Overwrite, Nil, CompressionType.GZip)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h1
		Protected Function GUnZip(Source As FolderItem, Destination As Writeable) As Boolean
		  ...
		  ' calls Inflate(FolderItem, Writeable, Integer, CompressionType) As Boolean
		  Return Inflate(Source, Destination, Nil, CompressionType.GZip)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h1
		Protected Function GUnZip(Source As MemoryBlock) As MemoryBlock
		  ...
		  ' calls Inflate(MemoryBlock, MemoryBlock, CompressionType) As MemoryBlock
		  Return Inflate(Source, Nil, CompressionType.GZip)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h1
		Protected Function GUnZip(Source As MemoryBlock, Destination As FolderItem, Overwrite As Boolean = False) As Boolean
		  ...
		  ' calls Inflate(MemoryBlock, FolderItem, Boolean, MemoryBlock, CompressionType) As Boolean
		  Return Inflate(Source, Destination, Overwrite, Nil, CompressionType.GZip)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h1
		Protected Function GUnZip(Source As MemoryBlock, Destination As Writeable) As Boolean
		  ...
		  ' calls Inflate(MemoryBlock, Writeable, MemoryBlock, CompressionType) As Boolean
		  Return Inflate(Source, Destination, Nil, CompressionType.GZip)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h1
		Protected Function GUnZip(Source As Readable) As MemoryBlock
		  ...
		  ' calls Inflate(Readable, MemoryBlock, CompressionType) As MemoryBlock
		  Return Inflate(Source, Nil, CompressionType.GZip)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h1
		Protected Function GUnZip(Source As Readable, Destination As FolderItem, Overwrite As Boolean = False) As Boolean
		  ...
		  ' calls Inflate(Readable, FolderItem, Boolean, MemoryBlock, CompressionType) As Boolean
		  Return Inflate(Source, Destination, Overwrite, Nil, CompressionType.GZip)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h1
		Protected Function GUnZip(Source As Readable, Destination As Writeable) As Boolean
		  ...
		  ' calls Inflate(Readable, Writeable, MemoryBlock, CompressionType) As Boolean
		  Return Inflate(Source, Destination, Nil, CompressionType.GZip)
		End Function
	#tag EndMethod

	#tag Method, Flags = &h1
		Protected Function GZip(Source As FolderItem, Destination As FolderItem, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default, Overwrite As Boolean = False) As Boolean
		  ' GZip the Source file into the Destination file. Use GUnZip to reverse.
		  ' See: https://github.com/charonn0/RB-zlib/wiki/zlib.GZip
		  
		  ' calls Deflate(FolderItem, FolderItem, CompressionLevel, Boolean, CompressionType) As Boolean
		  Return Deflate(Source, Destination, Level, Overwrite, CompressionType.GZip)
		End Function
	#tag EndMethod

	#tag Method, Flags = &h1
		Protected Function GZip(Source As FolderItem, Destination As Writeable, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default) As Boolean
		  ' GZip the Source file into the Destination stream. Use GUnZip to reverse.
		  ' See: https://github.com/charonn0/RB-zlib/wiki/zlib.GZip
		  
		  ' calls Deflate(FolderItem, Writeable, CompressionLevel, CompressionType) As Boolean
		  Return Deflate(Source, Destination, Level, CompressionType.GZip)
		  
		End Function
	#tag EndMethod

	#tag Method, Flags = &h1
		Protected Function GZip(Source As FolderItem, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default) As MemoryBlock
		  ' GZip the Source file and return it. Use GUnZip to reverse.
		  ' See: https://github.com/charonn0/RB-zlib/wiki/zlib.GZip
		  
		  ' calls Deflate(FolderItem, CompressionLevel, CompressionType) As MemoryBlock
		  Return Deflate(Source, Level, CompressionType.GZip)
		End Function
	#tag EndMethod

	#tag Method, Flags = &h1
		Protected Function GZip(Source As MemoryBlock, Destination As FolderItem, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default, Overwrite As Boolean = False) As Boolean
		  ' GZip the Source data into the Destination file. Use GUnZip to reverse.
		  ' See: https://github.com/charonn0/RB-zlib/wiki/zlib.GZip
		  
		  ' calls Deflate(MemoryBlock, FolderItem, CompressionLevel, Boolean, CompressionType) As Boolean
		  Return Deflate(Source, Destination, Level, Overwrite, CompressionType.GZip)
		End Function
	#tag EndMethod

	#tag Method, Flags = &h1
		Protected Function GZip(Source As MemoryBlock, Destination As Writeable, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default) As Boolean
		  ' GZip the Source data into the Destination stream. Use GUnZip to reverse.
		  ' See: https://github.com/charonn0/RB-zlib/wiki/zlib.GZip
		  
		  Dim src As New BinaryStream(Source)
		  ' calls Deflate(Readable, Writeable, CompressionLevel, CompressionType) As Boolean
		  Return Deflate(src, Destination, Level, CompressionType.GZip)
		End Function
	#tag EndMethod

	#tag Method, Flags = &h1
		Protected Function GZip(Source As MemoryBlock, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default) As MemoryBlock
		  ' GZip the Source data and return it. Use GUnZip to reverse.
		  ' See: https://github.com/charonn0/RB-zlib/wiki/zlib.GZip
		  
		  ' calls Deflate(MemoryBlock, CompressionLevel, CompressionType) As MemoryBlock
		  Return Deflate(Source, Level, CompressionType.GZip)
		End Function
	#tag EndMethod

	#tag Method, Flags = &h1
		Protected Function GZip(Source As Readable, Destination As FolderItem, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default, Overwrite As Boolean = False) As Boolean
		  ' GZip the Source stream into the Destination file. Use GUnZip to reverse.
		  ' See: https://github.com/charonn0/RB-zlib/wiki/zlib.GZip
		  
		  ' calls Deflate(Readable, FolderItem, CompressionLevel, Boolean, CompressionType) As Boolean
		  Return Deflate(Source, Destination, Level, Overwrite, CompressionType.GZip)
		End Function
	#tag EndMethod

	#tag Method, Flags = &h1
		Protected Function GZip(Source As Readable, Destination As Writeable, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default) As Boolean
	...
	#tag Method, Flags = &h1
		Protected Function GZip(Source As Readable, Level As zlib.CompressionLevel = zlib.CompressionLevel.Default) As MemoryBlock
		  ' GZip the Source stream and return it. Use GUnZip to reverse.
		  ' See: https://github.com/charonn0/RB-zlib/wiki/zlib.GZip
		  
		  ' calls Deflate(Readable, CompressionLevel, CompressionType) As MemoryBlock
		  Return Deflate(Source, Level, CompressionType.GZip)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h1
		Protected Function Inflate(Source As FolderItem, Destination As FolderItem, Overwrite As Boolean = False, Dictionary As MemoryBlock = Nil, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As Boolean
	...
	#tag Method, Flags = &h1
		Protected Function Inflate(Source As FolderItem, Dictionary As MemoryBlock = Nil, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As MemoryBlock
	...
	#tag Method, Flags = &h1
		Protected Function Inflate(Source As FolderItem, Destination As Writeable, Dictionary As MemoryBlock = Nil, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As Boolean
	...
	#tag Method, Flags = &h1
		Protected Function Inflate(Source As MemoryBlock, Destination As FolderItem, Overwrite As Boolean = False, Dictionary As MemoryBlock = Nil, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As Boolean
	...
	#tag Method, Flags = &h1
		Protected Function Inflate(Source As MemoryBlock, Dictionary As MemoryBlock = Nil, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As MemoryBlock
	...
	#tag Method, Flags = &h1
		Protected Function Inflate(Source As MemoryBlock, Destination As Writeable, Dictionary As MemoryBlock = Nil, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As Boolean
	...
	#tag Method, Flags = &h1
		Protected Function Inflate(Source As Readable, Destination As FolderItem, Overwrite As Boolean = False, Dictionary As MemoryBlock = Nil, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As Boolean
	...
	#tag Method, Flags = &h1
		Protected Function Inflate(Source As Readable, Dictionary As MemoryBlock = Nil, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As MemoryBlock
	...
	#tag Method, Flags = &h1
		Protected Function Inflate(Source As Readable, Destination As Writeable, Dictionary As MemoryBlock = Nil, Encoding As zlib.CompressionType = zlib.CompressionType.Deflate) As Boolean
	...
	#tag Constant, Name = Z_NEED_DICT, Type = Double, Dynamic = False, Default = \"2", Scope = Private
	#tag EndConstant
	...
	#tag Enum, Name = CompressionLevel, Type = Integer, Flags = &h1
		None=0
		  Fastest=1
		  Faster=2
		  Fast=3
	...
	#tag Enum, Name = CompressionStrategy, Type = Integer, Flags = &h1
		Default=0
		  Filtered=1
		  HuffmanOnly=2
		  RLE=3
	...
	#tag Enum, Name = FlushLevel, Type = Integer, Flags = &h1
		NoFlush=0
		  Partial=1
		  Sync=2
		  Full=3
